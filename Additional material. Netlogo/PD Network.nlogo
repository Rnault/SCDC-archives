...
to setup
  ...
  ifelse n-type = "sf" [network_BA-model] [network_WS-model]
...
to make-turtles
  ;;create our turtles
  create-turtles num-nodes [ ifelse random-float 1 < coop-frequency
    [set color blue] ;;; blue being cooperator
    [set color red] ];;; red being defector
...
to growth [partner]
  ...
  create-turtles 1
  [
    ifelse random-float 1 < coop-frequency
    [set color blue];;;blue being cooperator
    [set color red];;;red being defector
    if partner != nobody
...
;;;;;;;;;;; Actual Game: Prisonner's Dilemma ;;;;;;;;;;;;;;;;;;;;;;;;;

to initialize-variables
  ;;for dilemma to exist we need DC > CC > DD > CD, but 2CC > DC + CD
  ;;defecting as the short term solution and cooperating the long term one
  set payoff-CC 1
  set payoff-CD 0
  set payoff-DC 2
  set payoff-DD 0
...
to interact
  let partner (one-of (link-neighbors with [played? = false]))
  ...
  if partner != nobody [
    ...
    [
      ...
      [
        set score  (score + payoff-CC)
        ask partner [set score (score + payoff-CC)]
      ]
      [
        set score (score + payoff-CD)
        ask partner [set score (score + payoff-DC)]
      ]
    ]
    [ ;if I'm a defector
      ifelse ([color] of partner) = blue
      [
        set score (score + payoff-DC)
        ask partner [set score (score + payoff-CD)]
      ]
      [
        set score (score + payoff-DD)
        ask partner [set score (score + payoff-DD)]
      ]
    ]
  set played? true
  ]
...
to update-strategy
  ask turtles[
    ...
    ifelse color = blue[
        ...
        if score < score-before [ set color red]]
    ]
    [
        if score != score-before [
          if score < score-before [ set color blue]]]]
        set score-before score]
end
...
"" ""
PENS
"collaborators" 1.0 0 -13345367 true "" "plot count turtles with [color = blue]"
...
137
NIL
count turtles with [played? = True]
17
1
11
